...
interface NavigationBarProps {
  ...
  setModelState: React.Dispatch<SetStateAction<{ [key: string]: boolean }>>;
}
...
const NavigationBar = ({ modelState, setModelState }: NavigationBarProps) => {
  ...
  return (
    <div
      className={`fixed left-1/2 text-white -translate-x-1/2 max-w-[900px] w-full h-12
        top-2 sm:top-5 rounded-t-lg bg-[#48484A]/50 z-20 flex justify-between rounded-bl-lg items-center px-2 sm:px-6 easy-in duration-150 ${
          !open ? "rounded-br-lg" : ""
        }`}
    >
      ...
      <ul className="w-[45%] flex justify-center space-x-10">
        <Menubar.Root
          className="flex"
          onValueChange={() => void setOpen(!open)}
        >
          <Menubar.Menu>
            <Menubar.Trigger className="inline-flex relative items-center">
              Components
              <div
                className={`-right-2.5 bg-green-500 absolute rounded-full easy-in duration-300 ${
                  open ? "h-1 w-1" : "h-2 w-0.5"
                }`}
              />
            </Menubar.Trigger>
            <AnimatePresence>
